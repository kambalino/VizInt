...
(async function () {
  ...
  const DEFAULT_ENABLED = ['daily','prayers','header','eom','worldtz','settings'];


  // Known gadgets for the Settings UI
    // Gadget catalog & title map (sourced from Registry.js)
  const GADGET_CATALOG = (window.Registry && window.Registry.GADGET_CATALOG) || [];
...
window.GADGETS.header = {
  ...
  function unmountAll(dock) {
	...
	function render() {
		...
		const settings = ensureDefaults(getSettings());  // ✅ apply defaults here
	...
	// Initialize default settings keys if missing
