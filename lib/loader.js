...
(async function () {
	...
	// Factory helpers: construct module instances, not direct window.* bindings.
	// These are the only places that peek at globals, and even then only as a
	// temporary migration aid. From Portal's perspective, ctx.libs is built from
	// explicit instances, not from window.* slots.

	function makeCoreInstance() {
		if (typeof window.makeCore === 'function') {
			return window.makeCore();
		}
		if (window.SHARED_CORE) {
			return window.SHARED_CORE;
		}
		if (window.Core) {
			console.warn('[VizInt] Core library found on window.Core; treating it as a legacy module instance. Prefer explicit factories in v1.3+.');
			return window.Core;
		}
		console.warn('[VizInt] Core library instance not found; ctx.libs.Core will be empty.');
		return {};
	}

	function makeAtlasInstance() {
		if (typeof window.makeAtlas === 'function') {
			return window.makeAtlas();
		}
		if (window.Atlas) {
			console.warn('[VizInt] Atlas library found on window.Atlas; treating it as a legacy module instance. Prefer explicit factories in v1.3+.');
			return window.Atlas;
		}
		console.warn('[VizInt] Atlas library instance not found; ctx.libs.Atlas will be empty.');
		return {};
	}

	function makeChronusInstance() {
		if (typeof window.makeChronus === 'function') {
			return window.makeChronus();
		}
		if (window.Chronus) {
			console.warn('[VizInt] Chronus library found on window.Chronus; treating it as a legacy module instance. Prefer explicit factories in v1.3+.');
			return window.Chronus;
		}
		console.warn('[VizInt] Chronus library instance not found; ctx.libs.Chronus will be empty.');
		return {};
	}

	function makeNexusInstance() {
		if (typeof window.makeNexus === 'function') {
			return window.makeNexus();
		}
		if (window.Nexus) {
			console.warn('[VizInt] Nexus library found on window.Nexus; treating it as a legacy module instance. Prefer explicit factories in v1.3+.');
			return window.Nexus;
		}
		console.warn('[VizInt] Nexus library instance not found; ctx.libs.Nexus will be empty (bus will fall back to window).');
		return {};
	}

	let SHARED_LIBS = null;

	// Build the shared-library surface (Core → Atlas → Chronus → Nexus).
	function buildLibs() {
		if (SHARED_LIBS) return SHARED_LIBS;

		const Core    = makeCoreInstance();
		const Atlas   = makeAtlasInstance();
		const Chronus = makeChronusInstance();
		const Nexus   = makeNexusInstance();

		SHARED_LIBS = { Core, Atlas, Chronus, Nexus };
		return SHARED_LIBS;
	}
	...
	// ---------- chrome builder ----------
	// returns { slot, body, iconEl, chipsSpan }
	...
	async function mountGadget(name, ctx, dock) {
		...
		// 3) Titlebar chips (optional; portal-level toggle, only for API 1.0 gadgets)
		const s = getSettings();
		...
		if (showChips && chipsSpan && isApiV1 && Array.isArray(manifest.capabilities)) {
			...
			// Ordered capability chips (chronus, atlas, network, served)
		...
		// 6) Info affordance via icon (always available)
		if (iconEl) {
