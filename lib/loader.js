...
(async function () {
  const KEY = 'portalSettings';
	...
	const GADGET_CATALOG = (window.REGISTRY?.GADGETS || []);  // keep full metadata
/*	TBRM:
...
window.GADGETS.header = {
  ...
  function buildChrome(name, dock, ctx) {
	...
	// icon box (emoji or png)
	if (gadgetInfo && (gadgetInfo.iconEmoji || gadgetInfo.iconPng)) {
		const box = document.createElement('span');
		box.className = 'g-iconbox';
		if (gadgetInfo.iconBg) box.style.background = gadgetInfo.iconBg;

		if (gadgetInfo.iconBorder) {
			box.style.border = `1px solid ${gadgetInfo.iconBorder}`;
		}
    ...
    //TBRM: already done above: title.textContent   = GADGET_TITLES[name] || name;
    btnInfo.title       = 'Info';
  ...
  function unmountAll(dock) {
	...
	async function mountGadget(name, ctx, dock) {
		const { slot, body, btnInfo } = buildChrome(name, dock, ctx);
