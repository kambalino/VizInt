...
(async function () {
	...
	function computeBadgesForGadget(meta) {
		if (!meta) return [];
	...
	async function mountGadget(gadgetId, ctx, dock) {
		...
		const label = meta?.label || gadgetId;

		const isHeader = (gadgetId === 'header');

		// Gadget type: single | multi | system
		// - manifest._type will refine this later, but for now:
		//   header + settings are treated as system gadgets.
		const metaType = meta && (meta._type || meta.type);
		const initialType =
			metaType ||
			((gadgetId === 'header' || gadgetId === 'settings') ? 'system' : 'single');

		const isSystem = (initialType === 'system');

		const descriptor = {
			id: gadgetId,
			classId: gadgetId,           // will diverge with multi-instance / manifest._class later
			instanceId: gadgetId,        // ditto for manifest._id
			instanceName: label,
			label,
			manifest: null,              // filled after load if needed
			gadgetType: initialType,     // 'single' | 'multi' | 'system'
			isSystem,
			isHeader,
			canClose: !isSystem,
			disableCloseReason: isSystem ? 'system gadget cannot be closed' : '',
			/// there's no way this is good code.
			supportsSettings: !!(meta && meta.supportsSettings),
			showSettingsGear: !!(meta && meta.supportsSettings), // Option A: registry drives gear visibility
			gadgetInfo: meta || null,
			badges: computeBadgesForGadget(meta),
			bidi: 'ltr'                  // will be overridden once manifest is known
		};

		// Chrome contract: PortalChrome must exist and export createGadgetChrome
		...
		// runtime callbacks for chrome interactions
		const chromeCtx = {
			onClose() {
			...
			onToggleFullscreen(isFs) {
				const s  = getSettings();
				const st = s.gadgetState || {};
				st[gadgetId] = { ...(st[gadgetId] || {}), fullscreen: !!isFs, collapsed: false };
				setSettings({ ...s, gadgetState: st });
			},
			...
			onThemeToggle: isHeader
				...
				: null,
			// Info affordance will be wired after manifest/API are known.
			onInfoRequest: null,
			...
			// Settings affordance – placeholder wired pre-chrome for registry-flagged gadgets
			onSettingsRequested: null
		};

		// Option A: ensure chrome sees a handler when registry says supportsSettings
		if (descriptor.supportsSettings && typeof chromeCtx.onSettingsRequested !== 'function') {
			chromeCtx.onSettingsRequested = function () {
				console.info('[VizInt] Settings requested (placeholder); awaiting manifest/onSettingsRequested for', gadgetId);
			};
		}
		...
		// Apply bidi from manifest (default ltr)
		const bidi = manifest.bidi || 'ltr';
		slot.setAttribute('dir', bidi);
		slot.classList.toggle('g-ltr', bidi === 'ltr');
		descriptor.bidi = bidi;

		// Build per-instance ctx (API 1.0) vs legacy ctx
		const env = {
			geometry: null,
			layout: null,
		...
		// Info icon affordance wiring (chrome → portal → api/manifest)
