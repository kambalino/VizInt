...
(async function () {
	...
	function normalizeEnabled(list) {
		...
		// NOTE (Option B, W48:15 / W48:20):
		// - "settings" is a *normal* gadget
		// - Do NOT force-enable it here; user controls its presence.
		// (No auto-append for "settings".)
	...
	async function mountGadget(gadgetId, ctx, dock) {
		...
		const chromeCtx = {
			onClose() {
				...
				// Multi-instance-aware path (future) — if Portal.removeInstance exists, prefer it.
				// This will remove just this instance, and Portal will decide what that means
				// for enabledGadgets / instance tables.
				if (window.Portal && typeof window.Portal.removeInstance === 'function') {
					try {
						window.Portal.removeInstance(descriptor.classId, descriptor.instanceId);
					} catch (e) {
						console.warn('[VizInt] Portal.removeInstance threw; falling back to legacy close.', e);
					}
					return;
				}

				// Legacy single-instance behavior (current dock model):
				// Treat close as "disable this gadget class" in enabledGadgets.
				const s    = getSettings();
			...
			// New canonical layout channel (W48:20)
			initialLayout,

			onLayoutChanged(nextLayout) {
				const layoutPatch = (nextLayout && typeof nextLayout === 'object')
					? nextLayout
					: {};

				// Canonical per-instance layout:
				// settings.layout[classId][instanceId] = { collapsed, spanWide, fullscreen, ... }
				writeLayoutForInstance(descriptor.classId, descriptor.instanceId, layoutPatch);

				// Optional: legacy gadgetState bridge for backwards compat with existing chrome.js.
				const s   = getSettings();
				const st  = s.gadgetState || {};
				const prev = st[gadgetId] || {};
				const legacyPatch = {};

				if (Object.prototype.hasOwnProperty.call(layoutPatch, 'collapsed')) {
					legacyPatch.collapsed = !!layoutPatch.collapsed;
				}
				if (Object.prototype.hasOwnProperty.call(layoutPatch, 'spanWide')) {
					legacyPatch.wide = !!layoutPatch.spanWide;
				}
				if (Object.prototype.hasOwnProperty.call(layoutPatch, 'fullscreen')) {
					legacyPatch.fullscreen = !!layoutPatch.fullscreen;
					if (layoutPatch.fullscreen) {
						legacyPatch.collapsed = false;
					}
				}

				st[gadgetId] = { ...prev, ...legacyPatch };
				setSettings({ ...s, gadgetState: st });
			},

			// Backwards-compat wrappers used by current chrome.js
	...
	function getInstances(classId) {
		...
		return snapshotInstancesForClass(key);
	}

	// --- Stage-2B: public multi-instance APIs (partial, no dock multi-instance yet) ---

	///! why are there two copies of this?
	function addInstance(classId) {
		if (typeof classId !== 'string' || !classId) {
			console.warn('[VizInt Portal] addInstance requires a non-empty classId.');
			return null;
		}
		const list = instancesByClass.get(classId);
		if (!list || !list.length) {
			console.warn('[VizInt Portal] addInstance: no existing instances for classId:', classId);
			return null;
		}

		const baseRec = list[0];
		const baseDesc = baseRec.descriptor;
		if (baseDesc && baseDesc.isSystem) {
			console.warn('[VizInt Portal] addInstance: system gadgets are single-instance and cannot be cloned:', classId);
			return null;
		}

		const baseName =
			baseRec.displayName ||
			(baseDesc && (baseDesc.instanceName || baseDesc.label || baseDesc.id)) ||
			classId;

		const baseSlug = normalizeSlug(baseName) || 'instance';
		const existingIds = new Set(list.map(r => r.instanceId));

		let candidateId = baseSlug;
		let n = 2;
		while (existingIds.has(candidateId)) {
			const nn = String(n).padStart(2, '0');
			candidateId = `${baseSlug}#${nn}`;
			n++;
		}

		const newDisplayName = `${baseName} #${list.length + 1}`;

		// Clone descriptor shallowly; deeper gadget-specific fields remain shared.
		const newDescriptor = {
			...baseDesc,
			instanceId: candidateId,
			instanceName: newDisplayName,
		...
		list.push(newRecord);
		recomputeInstanceIndexesForClass(classId);
		emitInstancesChangedForClass(classId);
	...
	function removeInstance(classId, instanceId) {
		if (typeof classId !== 'string' || !classId) {
			console.warn('[VizInt Portal] removeInstance requires a non-empty classId.');
			return;
		}
		if (typeof instanceId !== 'string' || !instanceId) {
			console.warn('[VizInt Portal] removeInstance requires a non-empty instanceId.');
			return;
		}

		const found = findInstanceRecord(classId, instanceId);
		if (!found) {
			console.warn('[VizInt Portal] removeInstance: instance not found:', classId, instanceId);
			return;
		}

		const { list, index, record } = found;
		const desc = record.descriptor || {};
		if (desc.isSystem) {
			console.warn('[VizInt Portal] removeInstance: system gadgets cannot be removed via multi-instance API:', classId);
			return;
		}

		// Delete per-instance settings for this (storageKey, instanceId) pair.
		if (desc.storageKey && record.instanceId) {
			deleteSettingsForStorageKeyAndInstance(desc.storageKey, record.instanceId);
		}

		list.splice(index, 1);
		if (!list.length) {
			instancesByClass.delete(classId);
		} else {
			recomputeInstanceIndexesForClass(classId);
		}
		emitInstancesChangedForClass(classId);
	...
	function renameInstance(classId, instanceId, newName) {
		if (typeof classId !== 'string' || !classId) {
			console.warn('[VizInt Portal] renameInstance requires a non-empty classId.');
			return;
		}
		if (typeof instanceId !== 'string' || !instanceId) {
			console.warn('[VizInt Portal] renameInstance requires a non-empty instanceId.');
			return;
		}
		const rawNewName = (newName == null) ? '' : String(newName);
		const trimmedName = rawNewName.trim();
		if (!trimmedName) {
			console.warn('[VizInt Portal] renameInstance: newName must be non-empty.');
			return;
		}
		...
		const desc = record.descriptor || {};
		if (desc.isSystem) {
			console.warn('[VizInt Portal] renameInstance: system gadgets cannot be renamed via multi-instance API:', classId);
			return;
		}

		const oldInstanceId = record.instanceId;

		// Derive a new instanceId from the name, ensuring uniqueness within this class.
		const baseSlug = normalizeSlug(trimmedName) || 'instance';
		const existingIds = new Set(list.map(r => r.instanceId));
		existingIds.delete(oldInstanceId); // allow keeping the same id if unchanged
		...
		const newInstanceId = candidateId;

		// Migrate settings on canonical path only; legacy Vz: keys left untouched for now.
		if (desc.storageKey && oldInstanceId !== newInstanceId) {
			renameSettingsForStorageKey(desc.storageKey, oldInstanceId, newInstanceId);
		}

		// Update record + descriptor.
		record.instanceId = newInstanceId;
		record.displayName = trimmedName;

		if (desc) {
			desc.instanceId = newInstanceId;
			desc.instanceName = trimmedName;
			desc.displayName = trimmedName;
		}

		// Index stays the same; recompute to be safe in case future flows touch ordering.
		recomputeInstanceIndexesForClass(classId);

		// Option A: emit after mutation + settings migration.
		const payload = {
			classId: classId,
		...
		window.dispatchEvent(new CustomEvent('portal:gadgetRenamed', {
			detail: payload
		}));
	...
	function reorderInstance(classId, instanceId, direction) {
		if (typeof classId !== 'string' || !classId) {
			console.warn('[VizInt Portal] reorderInstance requires a non-empty classId.');
			return;
		}
		if (typeof instanceId !== 'string' || !instanceId) {
			console.warn('[VizInt Portal] reorderInstance requires a non-empty instanceId.');
			return;
		}
		const dir = (direction === 'up' || direction === 'down') ? direction : null;
		if (!dir) {
			console.warn('[VizInt Portal] reorderInstance: direction must be "up" or "down".');
			return;
		}
	...
	window.Portal = {
		...
		addInstance,           // NEW (Stage-2C)
		renameInstance,        // NEW (Stage-2C)
		removeInstance,        // NEW (Stage-2C)
		getInstanceConfig,     // NEW (Stage-2C, optional)
		DEFAULT_ENABLED,
		GADGET_CATALOG,
		safeResetAll,
		openSettingsModal,
