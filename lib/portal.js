 ...
 *	$HISTORY:
	...
	2025-12-11	-	1.2.15	(U:Portal)
	- v1.4 scaffold: tileOrder reconciliation + reorderTile API
...
(async function () {
	...
	function computeInstanceIdentity(descriptor) {
		...
		const rawInstance = descriptor.instanceId || manifest._id || 'default';
	...
	function ensureTileOrderInitialized(settings, enabled, instances) {
		...
		const tileOrder   = Array.isArray(settings.tileOrder) ? settings.tileOrder.slice() : [];
		const seen        = new Set();

		function key(classId, instanceId) {
			return String(classId) + '::' + String(instanceId);
		}

		for (let i = 0; i < tileOrder.length; i++) {
			const t = tileOrder[i];
			if (!t || typeof t !== 'object') continue;
			if (!t.classId || !t.instanceId) continue;
			seen.add(key(t.classId, t.instanceId));
		}

		// Helper: insert new tile after the nearest existing tile with same classId (Decision 3),
		// else append at end.
		function insertAfterPeer(classId, tile) {
			let insertAt = -1;
			for (let i = tileOrder.length - 1; i >= 0; i--) {
				const t = tileOrder[i];
				if (t && t.classId === classId) { insertAt = i + 1; break; }
			}
			if (insertAt < 0 || insertAt > tileOrder.length) insertAt = tileOrder.length;
			tileOrder.splice(insertAt, 0, tile);
		}

		let changed = false;

		// 1) Ensure every enabled singleton/system gadget has a singleton tile entry.
		for (let i = 0; i < enabledList.length; i++) {
			const gadgetId = enabledList[i];
			if (!gadgetId || typeof gadgetId !== 'string') continue;

			// If this gadgetId has any instance buckets referencing it, we do NOT create a singleton tile.
			...
			if (instances && typeof instances === 'object') {
				for (const classId in instances) {
					...
					for (const recId in records) {
						...
						if (rec && rec.manifestId === gadgetId) { hasBuckets = true; break; }
					}
					if (hasBuckets) break;
				}
			}
			...
			const k = key(classId, '__singleton__');
			if (!seen.has(k)) {
				seen.add(k);
		...
		// 2) Ensure every persisted instance has a tileOrder entry (even if class isn't enabled yet).
		if (instances && typeof instances === 'object') {
			for (const classId in instances) {
				if (!Object.prototype.hasOwnProperty.call(instances, classId)) continue;
				const bucket = instances[classId];
				...
				const records = bucket.records || {};
				const order = Array.isArray(bucket.order) ? bucket.order.slice() : Object.keys(records);

				for (let j = 0; j < order.length; j++) {
					const instanceId = order[j];
					if (!instanceId) continue;
					const rec = records[instanceId];
					if (!rec) continue;

					const k = key(classId, instanceId);
					if (seen.has(k)) continue;

					seen.add(k);
					insertAfterPeer(classId, { classId, instanceId });
					changed = true;
				}
			}
		}
		...
		// Persist if:
		// - tileOrder was empty (seed), OR
		// - we added missing entries.
		if (changed || (!Array.isArray(settings.tileOrder) || settings.tileOrder.length === 0)) {
	...
	function reorderTile(classId, instanceId, dir) {
		const settings = ensureDefaults(getSettings());
		const tileOrder = Array.isArray(settings.tileOrder) ? settings.tileOrder.slice() : [];

		const idx = tileOrder.findIndex(t => t && t.classId === classId && t.instanceId === instanceId);
		...
		const j = idx + delta;
		if (j < 0 || j >= tileOrder.length) return false;

		// Policy: header tile is pinned to the very top (index 0).
		// No tile may move above it, and header itself cannot move down.
		const headerIdx = tileOrder.findIndex(t => t && t.classId === 'header' && t.instanceId === '__singleton__');

		// If header exists, prevent any move that results in a tile landing above it.
		if (headerIdx >= 0) {
			// Block moving header down.
			if (classId === 'header' && instanceId === '__singleton__') return false;

			// Block any move that would place the moving tile above the header.
			// (i.e., moving into an index < headerIdx)
			if (j < headerIdx) return false;

			// Also block swapping directly with header (equivalent, but clearer).
			const target = tileOrder[j];
			if (target && target.classId === 'header' && target.instanceId === '__singleton__') return false;
		}


		const tmp = tileOrder[idx];
		tileOrder[idx] = tileOrder[j];
		tileOrder[j] = tmp;

		setSettings(Object.assign({}, settings, { tileOrder }));

		window.dispatchEvent(new CustomEvent('portal:tileOrderChanged', {
			detail: { tileOrder: tileOrder.slice() }
		}));

		// Re-render to reflect ordering changes in the dock.
		render();
...
/*				// Singleton/system tile
				...
				if (tile.instanceId === '__singleton__') {
					await mountGadget(tile.classId, ctx, dock, {
						classId: tile.classId,
						instanceId: '__singleton__',
						displayName: tile.classId
					});
					continue;
				}
			...
			for (const [classId] of instancesByClass.entries()) {
				...
				if (rerenderQueued) {
					...
					// run one more render pass to pick up mutations that occurred during rendering
					render();
				}
		}
	}
	...
	window.Portal = {
        ...
        reorderInstance,
		reorderTile,
		// Stage-2C (new) – used by Settings MI child rows
