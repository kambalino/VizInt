	...
	async function mountGadget(gadgetId, ctx, dock) {
		...
		const chromeCtx = {
			...
			onLayoutChanged(nextLayout) {
				...
				// Write to settings.layout[classId][instanceId]
				writeLayoutForInstance(descriptor.classId, descriptor.instanceId, layoutPatch);

				// Optional: legacy gadgetState bridge for backwards compat.
				const s	= getSettings();
			...
			// (rest of chromeCtx unchanged: onInfoRequest, onSettingsRequested, instance rename/delete placeholders, etc.)
			// --- RESTORED HOOKS ---
