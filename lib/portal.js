...
(async function () {
	...
	// - Remove duplicates
	// - Force 'header' first (if present in catalog)
	// - Force 'settings' last (if present in catalog)
	function normalizeEnabled(list) {
		const knownIds = new Set(GADGET_CATALOG.map(g => g.id));
		const seen     = new Set();
		const out      = [];

		// 1) Start from last known list or DEFAULT_ENABLED, filter unknowns, dedupe.
		...
		// 2) Ensure header exists, but only prepend if missing.
		if (!seen.has('header') && knownIds.has('header')) {
			out.unshift('header');
			seen.add('header');
		}

		// 3) Ensure settings exists, but do NOT force it to the tail if user already
		//    placed it somewhere. Only append if missing.
	...
	async function mountGadget(gadgetId, ctx, dock) {
		...
		const isSettingsGadget = (gadgetId === 'settings');
		...
		const descriptor = {
			...
			isHeader,
			isSettingsGadget,
			canClose: !(isHeader || isSettingsGadget),
			disableCloseReason: isHeader
				? 'header cannot be closed'
				: (isSettingsGadget ? 'settings cannot be closed' : ''),
		...
		// Call into chrome.js to get shell (slot/body/etc.)
		const chromeCtx = {
			// runtime callbacks for chrome interactions
			onClose() {
				// Guard: settings + header cannot be removed
		...
		// Apply bidi from manifest (default ltr) to slot so chrome sees it
	...
	window.addEventListener('gadgets:update', () => {
		// Re-read settings and re-render on any structural change
		render();
	});
