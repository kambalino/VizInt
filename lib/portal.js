	...
	async function render() {
		...
		try {
			...
			for (const id of enabled) {
				// Find all class buckets whose records reference this gadget id as manifestId.
				...
				// No instance buckets found for this gadget:
				// → legacy single-instance behavior (one implicit instance).
				if (!bucketsForGadget.length) {
					await mountGadget(id, ctx, dock, null);
					continue;
				}

				// For each matching class bucket, fan out one tile per instanceId in order[].
				for (let b = 0; b < bucketsForGadget.length; b++) {
					...
					for (let i = 0; i < order.length; i++) {
						...
						const gadgetIdToUse = rec.manifestId || id;
