	...
	async function mountGadget(gadgetId, ctx, dock) {
		...
		const chromeCtx = {
			...
			onClose() {
				...
				// --- Stage-2C-aware path: only use removeInstance when an instance record exists ---
				const instRoot = s.instances && s.instances[descriptor.classId];
				...
				// If this class/instance lives in the instances substrate, treat it as multi-instance:
				if (
					hasInstanceRecord &&
					window.Portal &&
					typeof window.Portal.removeInstance === 'function'
				) {
					// Let Portal.removeInstance() handle:
					//   - settings.instances[classId].{order,records}
					//   - gadgetInstancesChanged
					//   - any internal book-keeping
