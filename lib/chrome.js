/*
	File: chrome.js
	Description: Gadget chrome/titlebar factory
	Author: U:UX & K&K
	$VER: 1.2.3
	
	HISTORY:
	2025-12-09 (U:UX/U:Portal)
	- Documented canonical DOM identity hooks (data-classid / data-instanceid) for multi-instance gadgets (49:13).
	- No functional changes; chrome remains render-only and Portal-owned fan-out.
	2025-11-30 (U:UX/U:Portal)
	- Switched to chromeCtx.initialLayout/onLayoutChanged for layout persistence (48:34);
	  fixed minimize/span-wide/fullscreen persistence bug.
	2025-11-29 (U:UX)
...
(function () {
	...
	function createGadgetChrome(descriptor, chromeCtx) {
		...
		// Canonical identity hooks for multi-instance UX
		if (descriptor && descriptor.classId) {
			slot.dataset.classid = descriptor.classId;
		}
		if (descriptor && descriptor.instanceId) {
			slot.dataset.instanceid = descriptor.instanceId;
		}
		...
		// ---------- state helpers (layout ↔ CSS; persistence via chromeCtx.onLayoutChanged) ----------

		const initialLayout = (chromeCtx && chromeCtx.initialLayout) || {};
		let collapsed  = !!initialLayout.collapsed;
		let wide       = !!initialLayout.spanWide;    // spanWide is the canonical flag
		let fullscreen = !!initialLayout.fullscreen;
		const hasExplicitLayout = !!initialLayout.hasExplicitState;

		function emitLayoutPatch(partial) {
			const fn = chromeCtx && chromeCtx.onLayoutChanged;
			if (typeof fn === 'function') {
				try { fn(partial); } catch (_) {}
			}
		}
		...
		function setCollapsed(isCollapsed) {
			...
			emitLayoutPatch({ collapsed });
		}

		function setFullWidth(isWide) {
			wide = !!isWide;
			slot.classList.toggle('g-spanwide', wide);
			emitLayoutPatch({ spanWide: wide });
		}

		function setFullscreen(isFs) {
			fullscreen = !!isFs;

			// New canonical class: g-fullscreen (keep g-maximized cleared for safety)
			slot.classList.toggle('g-fullscreen', fullscreen);
			slot.classList.toggle('g-maximized', false);

			if (fullscreen) {
				// Fullscreen implies not minimized
				collapsed = false;
				slot.classList.remove('g-minimized');
			}

			emitLayoutPatch({ fullscreen });
		}
	...
	function createSettingsModalShell(chromeCtx) {
		...
		// --- Header (compact, right-aligned X, using tiny controls) ---
		...
		// container: title left, close-right (flex)
		...
		// Close button (aligned right, micro button)
		const btnClose = doc.createElement('button');
		btnClose.type = 'button';
		btnClose.className = 'gbtn gbtn-xs vi-modal-close';
